...
// Backend URL — reads from .env (VITE_API_URL) or falls back to Vercel backend
const API_BASE = import.meta.env.VITE_API_URL || 'https://backend-eight-lovat-39.vercel.app'
...
export default function Practice({ user }) {
  ...
  const lastAudioUrl   = useRef(null)
  ...
  const processAudio = async (audioBlob) => {
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: Date.now() + 1, role: 'ai', text: data.reply || '', correction: data.correction || null, originalText: transcript, score: data.score || null, tip: data.tip || '' },
      ])
      ...
      if (data.reply?.trim()) {
        ...
        } catch (ttsErr) { console.warn('TTS error (non-fatal):', ttsErr) }
      }
    } catch (e) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0f172a', color: 'white' }}>
      ...
      <div style={{ background: '#1e1b4b', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button onClick={replayAudio} disabled={!lastAudioUrl.current} title="Replay last AI voice"
            style={{ background: lastAudioUrl.current ? 'rgba(99,102,241,0.25)' : 'rgba(255,255,255,0.05)', border: lastAudioUrl.current ? '1px solid rgba(99,102,241,0.5)' : '1px solid rgba(255,255,255,0.1)', color: lastAudioUrl.current ? '#a5b4fc' : 'rgba(255,255,255,0.25)', cursor: lastAudioUrl.current ? 'pointer' : 'not-allowed', padding: '6px 14px', borderRadius: '20px', fontSize: '14px', fontWeight: '600' }}>
            &#x1F509; Replay
          </button>
          <button onClick={() => setShowMalayalam((v) => !v)}
            style={{ background: showMalayalam ? 'rgba(234,179,8,0.2)' : 'rgba(255,255,255,0.08)', border: showMalayalam ? '1px solid rgba(234,179,8,0.4)' : '1px solid rgba(255,255,255,0.15)', color: showMalayalam ? '#fde68a' : 'rgba(255,255,255,0.6)', cursor: 'pointer', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '600' }}>
            &#x1F1EE;&#x1F1F3; Malayalam
          </button>
        </div>
      </div>
        ...
        {messages.map((msg) => {
          if (msg.role === 'user') return (
            <div key={msg.id} style={{ alignSelf: 'flex-end', maxWidth: '75%' }}>
              <div style={{ background: '#6366f1', borderRadius: '18px 18px 4px 18px', padding: '12px 16px', fontSize: '14px', lineHeight: '1.6' }}>{msg.text}</div>
            </div>
          )
          ...
          return (
            <div key={msg.id} style={{ alignSelf: 'flex-start', maxWidth: '80%' }}>
              ...
              <div style={{ background: '#1e2a3a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '18px 18px 18px 4px', padding: '14px 16px', fontSize: '14px', lineHeight: '1.6', color: 'rgba(255,255,255,0.9)' }}>{msg.text}</div>
              {hasCorrection && (
              ...
              {badge && <div style={{ marginTop: '6px' }}><span style={{ display: 'inline-block', background: badge.bg, border: `1px solid ${badge.border}`, color: badge.color, borderRadius: '20px', padding: '3px 10px', fontSize: '11px', fontWeight: '600' }}>{badge.label}</span></div>}
              {msg.tip && <div style={{ marginTop: '6px', background: 'rgba(99,102,241,0.15)', borderRadius: '8px', padding: '8px 12px', fontSize: '12px', color: '#c7d2fe', lineHeight: '1.5' }}>&#x1F4A1; <strong>Tip:</strong> {msg.tip}</div>}
            </div>
          )
        })}
        {isProcessing && (
      ...
      {showMalayalam && aiData?.malayalam_hint && (
        <div style={{ background: 'rgba(234,179,8,0.15)', border: '1px solid rgba(234,179,8,0.3)', borderRadius: '12px', padding: '12px 16px', margin: '0 20px 8px', color: '#fde68a', fontSize: '14px', lineHeight: '1.6', flexShrink: 0 }}>
          <strong style={{ fontSize: '11px', letterSpacing: '0.5px', textTransform: 'uppercase', opacity: 0.7 }}>Malayalam</strong><br />{aiData.malayalam_hint}
        </div>
      )}
      ...
      <div style={{ background: '#0f172a', padding: '20px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
        <button onMouseDown={startRecording} onMouseUp={stopRecording}
          ...
          disabled={isProcessing} aria-label={isRecording ? 'Stop recording' : 'Start recording'}
          style={{ width: '80px', height: '80px', borderRadius: '50%', border: isRecording ? '2px solid #f87171' : '2px solid rgba(255,255,255,0.2)', background: isRecording ? '#ef4444' : isProcessing ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.1)', boxShadow: isRecording ? '0 0 0 10px rgba(239,68,68,0.2)' : 'none', fontSize: '30px', cursor: isProcessing ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s', animation: isRecording ? 'pulse 1.5s ease-in-out infinite' : 'none', opacity: isProcessing ? 0.5 : 1, userSelect: 'none', WebkitUserSelect: 'none' }}>
          {isRecording ? '\u23F9' : '\uD83C\uDFA4'}
        </button>
